...
%method -- this will allow for a larger payload capability due to the fact
%that stall speed is the most sensitive parameter in this constraint space 
...
cdo=0.035; %estimated parasitic drag --can range from 0.025-0.04

AR=7.8; %estimated design AR only for sake of constraint space graph -based on this class of aircraft
...
Wp=(164.1/1000); %estimated payload weight - kilograms
Wto=Wp/0.201; %estimated allowable takeoff weight - based on historical data of small UAS
...
ylim([0,0.06]); legend('Max Loading/Turn','Endurance','Cruise','Takeoff Distance','Stall Condition');
ylabel('HP/W');xlabel('W/S');title('Constraint Space - Horsepower to Weight Ratio vs Wing Loading');
hold off
...
ylim([0,0.06]); legend('Max Loading/Turn','Endurance','Cruise','Takeoff Distance','Stall Condition','Design Point');
ylabel('HP/W');xlabel('W/S');title('Constraint Space - Horsepower to Weight Ratio vs Wing Loading');
hold off
...
b=sqrt(S*dAR)
CLo=Wto/(.5*rho*V^2*S) %required operating lift coefficient setting lift equal to weight
...
alpha=0:0.1:14;
CL2=2*pi.*(alpha*(pi/180)); %convert AOA to radians for calculation -- this is an estimate based on prandtl's flat plate theory for airfoils
CDi=CL2.^2./(pi*e*dAR);
CD=cdo+CDi;
LDrat=CL2./CD;
...
alphao=CLo/(2*pi*(pi/180)) %operating AOA (should be relatively close or the same as AOA from XFLR5 analysis)
CDoi=CLo^2/(pi*e*dAR);
CDo=cdo+CDoi; 
ldo=CLo/CDo
...
%Note : I think it would be interesting to see how fluctuating assumed cdo
%may affect results.  Perhpas we can come up with a way to decrease drag to
%improve the performancce
